...
module.exports = {
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("busqueda").setDescription("titulo").setRequired(true)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
  ...
  execute: async ({ client, interaction }) => {
    ...
    if (!interaction.member.voice.channel)
      return interaction.reply("Metete en llamaradas gil");
    ...
    if (interaction.options.getSubcommand() === "song") {
      ...
      embed
        .setDescription(`**[${song.title}](${song.url})** agregado a la lista`)
      ...
      // Search for the song using the discord-player
      let url = interaction.options.getString("busqueda");
      const result = await client.player.search(url, {
        requestedBy: interaction.user,
